...
import (
	...
	"github.com/pluralsh/deployment-operator/pkg/deploymentaccess"
...
func main() {
	...
	if err = (&deploymentaccess.Reconciler{
		Client:            mgr.GetClient(),
		Log:               ctrl.Log.WithName("controllers").WithName("DeploymentAccess"),
		DriverName:        info.Name,
		ProvisionerClient: provisionerClient,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "DeploymentAccess")
		os.Exit(1)
	}
